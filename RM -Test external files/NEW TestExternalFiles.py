...
def main():
  ...
  if not os.path.exists(IniFile):
    # 1 of 4 errors that goes to the command line console window
  ...
  try:
    ...
    # 2 of 4 errors that goes to the command line console window
    print("ERROR: The " + IniFileName
          + " file contains a format error and cannot be parsed.\n\n" )
    input("Press the <Enter> key to exit...")
    return 1

  # Set up the report file first.
  try:
    report_Path   = config['FILE_PATHS']['REPORT_FILE_PATH']
  except:
    # 3 of 4 errors that goes to the command line console window
  ...
  try:
    ...
    # 4 of 4 errors that goes to the command line console window
    print("ERROR: Cannot create the report file " + report_Path + "\n\n")
    input("Press the <Enter> key to exit...")
    return 1

  # all error messages, from this point forward, go to the report file.
  # Console window should close.
  ...
  with open( report_Path,  mode='w', encoding='utf-8-sig') as reportF:
    ...
    with create_DBconnection(database_Path, reportF) as dbConnection:
      reportF.write ("Report generated at      = "
                    + TimeStampNow() + "\n")
      reportF.write ("Database processed       = "
...
def ListUnReferencedFilesFeature(config, dbConnection, reportF):
  ...
  if not Path(ExtFilesFolderPath).exists():
    reportF.write ("ERROR: Directory path not found:"
                  + G_QT + ExtFilesFolderPath + G_QT + "\n")
    sys.exit()
  if not Path(ExtFilesFolderPath).is_dir():
    reportF.write ("ERROR: Path is not a directory:"
                  + G_QT + ExtFilesFolderPath + G_QT + "\n")
    sys.exit()
  ...
  reportF.write("    Folder processed: " + G_QT + ExtFilesFolderPath
               + G_QT + "\n")
  ...
  reportF.write("    # DB links minus # non-ignored files: "
